...
import { createPartnerEnvironmentScopeResolver } from "./resolveAuthoritativePartnerEnvironmentScope";
import {
...
function officeInviteRolesFromBody(
  value: unknown,
): readonly PlatformRole[] | null {
  if (!Array.isArray(value) || value.length === 0) return null;
  const roles = value.filter(
    (role): role is PlatformRole =>
      typeof role === "string" &&
      OFFICE_PARTNER_INVITE_ROLES.has(role as PlatformRole),
  );
    ...
    try {
      if (
        ...
        if (request.method === "PUT") {
          ...
          if (
            ...
            return respond(response, 400, {
              error: "Neplatná relace rozhodnutí.",
            });
          }
          ...
          try {
            ...
            if (error instanceof DecisionSessionScopeMismatchError) {
              return respond(response, 409, {
                error: "Relace rozhodnutí nepatří tomuto domu.",
              });
            }
            return respond(response, 400, {
              error: "Neplatná relace rozhodnutí.",
        ...
        const decisionSessionId =
          url.searchParams.get("decisionSessionId")?.trim() ?? "";
        ...
        if (record === null) {
          return respond(response, 404, {
            error: "Relace rozhodnutí neexistuje.",
          });
        }
        return respond(response, 200, {
          decisionSessionId: record.decisionSessionId,
      ...
      if (request.method === "POST" && path === "/public/leads") {
        ...
        if (
          ...
          ((contact as Record<string, unknown>).name as string).trim()
            .length === 0 ||
          typeof (contact as Record<string, unknown>).email !== "string" ||
          ((contact as Record<string, unknown>).email as string).trim()
            .length === 0 ||
          ...
          ((consent as Record<string, unknown>).acceptedAt as string).trim()
            .length === 0 ||
          typeof (consent as Record<string, unknown>).privacyUrl !== "string" ||
          ((consent as Record<string, unknown>).privacyUrl as string).trim()
            .length === 0 ||
          typeof (consent as Record<string, unknown>).privacyVersion !==
            "string" ||
          ((consent as Record<string, unknown>).privacyVersion as string).trim()
            .length === 0
        ) {
          return respond(response, 400, { error: "Neplatná poptávka." });
        }
        ...
        try {
          ...
          return respond(response, 400, {
            error: "Neplatný rozsah nebo chybějící zásady soukromí partnera.",
          });
        }
        if (body.consent?.privacyUrl !== scope.privacyUrl) {
          return respond(response, 400, {
            error: "Zásady soukromí neodpovídají partnerovi.",
          });
        }
        ...
        if (requestedSessionId !== undefined && requestedSessionId !== null) {
          ...
          const sessionRecord = await decisionSessions.getById(
      ...
      if (request.method === "GET" && path === "/partner/leads") {
        ...
        if (
          ...
          return respond(response, 403, {
            error: "Přístup k poptávkám není povolen.",
          });
        }
      ...
      if (request.method === "GET" && path === "/partner/decision-sessions") {
        ...
        if (
          ...
          return respond(response, 403, {
            error: "Přístup k relacím rozhodnutí není povolen.",
          });
        }
      ...
      if (projectConfigMatch !== null) {
        ...
        if (request.method === "PUT") {
          ...
          try {
            ...
            return respond(response, 400, {
              error: "Neplatná adresa zásad ochrany osobních údajů.",
            });
          }
        }
      ...
      if (publicCompanyContactMatch !== null && request.method === "GET") {
        ...
        const canonicalCompanyId =
          canonicalCompanyIdForOfficePartner(companyId);
      ...
      if (path === "/office/partners" || path.startsWith("/office/partners/")) {
        ...
        if (environmentScopeMatch !== null) {
          ...
          const scope = parsePartnerEnvironmentScope(
            await requestBody(request),
          );
          if (scope === null) {
            return respond(response, 400, {
        ...
        if (partnerMatch !== null) {
          ...
          if (request.method === "PUT") {
            ...
            const draft = officePartnerDraftFromBody(
              await requestBody(request),
            );
            if (draft === null) {
      ...
      if (path === "/office/invites" || path.startsWith("/office/invites/")) {
        ...
        if (request.method === "POST" && path === "/office/invites") {
          ...
          if (
            (await partnerSessions.findAccountByEmail(draft.email)) !== null
          ) {
            return respond(response, 409, {
      ...
      if (
        ...
        const token = decodeURIComponent(
          path.slice("/public/auth/activate/".length),
        );
      ...
      if (request.method === "POST" && path === "/public/auth/login") {
        ...
        if (issued === null)
          return respond(response, 401, { error: "Neplatné přihlášení." });
        setPartnerSessionCookie(response, issued.token, issued.expiresAt);
        return respond(response, 200, { ok: true, session: issued.identity });
      }
      ...
      if (housePackageMatch !== null || housePackageMediaMatch !== null) {
        ...
        if (housePackageMediaMatch !== null) {
          ...
          if (request.method === "POST") {
            ...
            if (
              ...
              return respond(response, 400, {
                error: "Media content-type is required.",
              });
            }
        ...
        if (action === "persist" && request.method === "POST") {
          ...
          if ("packageRoot" in body) {
            return respond(response, 400, {
              error: "packageRoot is server-owned.",
            });
          }
          if (body.files === null || typeof body.files !== "object") {
            return respond(response, 400, {
              error: "House Package files are required.",
            });
          }
      ...
      if (
        ...
        if (!isPlatformAdmin(current.user.roles)) {
          return respond(response, 403, {
            error: "Přidávat domy do registru může pouze CONIS Admin.",
          });
        }
        ...
        if (!body.id || !body.canonicalProjectId || !body.name) {
          return respond(response, 400, {
            error: "id, canonicalProjectId a name jsou povinné.",
          });
        }
        const saved =
          await canonicalRegistryAuthorityRepository.upsertHouseAuthority({
            id: body.id.trim(),
            ...
            ...(typeof body.slug === "string"
              ? { slug: body.slug.trim() }
              : {}),
            ...(typeof body.packageRoot === "string"
            ...
            ...(typeof body.dataMode === "string"
              ...
              : {}),
            ...(body.referenceProvenance !== undefined &&
            typeof body.referenceProvenance.sourceId === "string" &&
            typeof body.referenceProvenance.sourceVersion === "string"
              ? {
                  referenceProvenance: {
                    sourceId: body.referenceProvenance.sourceId,
                    sourceVersion: body.referenceProvenance.sourceVersion,
                  },
                }
              : {}),
          });
      ...
      if (
        request.method === "POST" &&
        path === "/public/auth/canonical-partner-authority"
      ) {
        const token = requestCookie(request, PARTNER_SESSION_COOKIE);
        ...
        if (!isPlatformAdmin(current.user.roles)) {
          return respond(response, 403, {
            error: "Canonical Partner může registrovat pouze CONIS Admin.",
          });
        }

        const body = (await requestBody(
          request,
        )) as import("./canonicalRegistryAuthorityRepository").CanonicalRegistryPartnerAuthorityBundle;

        try {
          const authority =
            await canonicalRegistryAuthorityRepository.upsertPartnerAuthority(
              body,
            );
          ...
          return respond(response, 400, {
            error:
              error instanceof Error
                ? error.message
                : "Canonical Partner se nepodařilo registrovat.",
          });
        }
      }
      ...
      if (
        ...
        const body = (await requestBody(
          request,
        )) as import("./canonicalRegistryAuthorityRepository").CanonicalRegistryAuthorityBundle;

        try {
          const projectId =
            typeof body?.project?.id === "string" ? body.project.id.trim() : "";

          const existing =
            projectId.length === 0
              ? null
              : await canonicalRegistryAuthorityRepository.resolveProjectAuthority(
                  projectId,
                );
          ...
          const authority =
            await canonicalRegistryAuthorityRepository.upsertAuthorityBundle(body);
      ...
      if (request.method === "POST" && path === "/public/auth/context") {
        ...
        const mutation = (await requestBody(
          request,
        )) as import("./partnerSessionRepository").PartnerSessionContextMutation;
        const session = await partnerSessions.mutateContext(token, mutation);

        if (session === null) {
          return respond(response, 403, {
            error:
              mutation.action === "switch" && mutation.projectId !== undefined
      ...
      if (
        ...
        return respond(response, 403, {
          error: "Local-pilot access requires loopback.",
        });
      }
      if (
        request.method === "POST" &&
        path === "/local-pilot/social-proof/events"
      ) {
        await socialProofRepository.record(
          (await requestBody(
            request,
          )) as import("./socialProofAnalytics").SocialProofAnalyticsEventInput,
        );
        return respond(response, 202, { accepted: true });
      }
      if (
        request.method === "GET" &&
        path === "/local-pilot/social-proof/aggregate"
      ) {
        const url = new URL(request.url ?? "/", "http://localhost");
        ...
        if (
          ...
          return respond(response, 400, {
            error: "companyId, projectId, houseId, from a to jsou povinné.",
          });
        }
        return respond(
          response,
          200,
          await socialProofRepository.aggregateHouse({
            companyId,
            projectId,
            houseId,
            from,
      ...
      if (
        request.method === "GET" &&
        path === "/local-pilot/social-proof/recent"
      ) {
        const url = new URL(request.url ?? "/", "http://localhost");
        const companyId = url.searchParams.get("companyId");
        const projectId = url.searchParams.get("projectId");
        ...
        if (
          ...
          return respond(response, 400, {
            error: "companyId, projectId, from a to jsou povinné.",
          });
        }
        const minimumVisitors = Number.parseInt(
          url.searchParams.get("minimumVisitors") ?? "2",
          10,
        );
        return respond(
          response,
          ...
          await socialProofRepository.recentActivity({
            ...
            from,
            to,
            minimumVisitors: Number.isFinite(minimumVisitors)
              ? Math.max(2, minimumVisitors)
              : 2,
          }),
        );
      }
      if (request.method === "POST" && path === "/local-pilot/orders") {
        ...
        if (!authorized)
          return respond(response, 403, {
            error: "Offer write capability is not valid for this order.",
          });
        return respond(response, 201, await orderRepository.create(orderInput));
      }
      if (
        request.method === "POST" &&
        path === "/local-pilot/offer-write-capabilities"
      ) {
        return respond(
          response,
          201,
          await offerWriteTokens.getOrIssue(
            (await requestBody(
              request,
            )) as import("./offerWriteTokenRepository").OfferWriteCapabilityScope,
          ),
        );
      }
      const proformaPdfMatch = path.match(
        /^\/local-pilot\/orders\/([^/]+)\/proforma\/pdf$/,
      );
      if (request.method === "GET" && proformaPdfMatch !== null) {
        ...
        if (order === null || proforma === null) {
          return respond(response, 404, {
            error: "Objednávka nebo proforma neexistuje.",
          });
        }
      ...
      if (orderProformaMatch !== null) {
        ...
        if (request.method === "POST") {
          ...
          if (
            token === null ||
            !(await offerWriteTokens.verifyOrder(token, orderId))
          ) {
            return respond(response, 401, {
              error: "Offer write capability is required.",
            });
        ...
        if (request.method === "GET") {
          ...
          return respond(
            response,
            proforma === null ? 404 : 200,
            proforma ?? { error: "Proforma neexistuje." },
          );
        }
      }
      if (
        request.method === "GET" &&
        path.startsWith("/local-pilot/proformas/")
      ) {
        const proformaId = decodeURIComponent(
          path.slice("/local-pilot/proformas/".length),
        );
      ...
      if (request.method === "GET" && path.startsWith("/local-pilot/orders/")) {
        const orderId = decodeURIComponent(
          path.slice("/local-pilot/orders/".length),
        );
        const order = await orderRepository.getByOrderId(orderId);
        return respond(
          response,
          order === null ? 404 : 200,
          order ?? { error: "Objednávka neexistuje." },
        );
      }
      if (request.method === "POST" && path === "/local-pilot/invites") {
        return respond(
          ...
          201,
          await repository.create(
            (await requestBody(request)) as PlatformInviteScope,
          ),
        );
      }
      const privateMatch = path.match(
        /^\/local-pilot\/invites\/([^/]+)\/(reissue|revoke)$/,
      );
      if (request.method === "POST" && privateMatch !== null) {
        ...
        return respond(
          response,
          result === null ? 404 : 200,
          result ?? { error: "Pozvánka není dostupná." },
        );
      }
