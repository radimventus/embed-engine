...
const snapshot: ClientOutputSnapshot = { schemaVersion:1,capturedAt:'2026-09-14T08:00:00.000Z',company:{id:'c',name:'Partner'},project:{id:'p',name:'Projekt'},house:{id:'h',name:'Bungalov 4KK',storeys:1},knowledgeVersion:'v04',variant:'UNIVERSAL',priorities:['Energie'],exterior:[{id:'e',url:'/house/exterior.webp',caption:'Exteriér ve vztahu k zahradě.'}],floorPlans:[{id:'f',url:'/house/floorplan.png',caption:'Jednopodlažní dispozice.'}],interiors:[{id:'i',url:'/house/interior.jpg',caption:'Obytný prostor a jeho každodenní využití.'}],priorityNarratives:[{title:'Energie',fact:'Doložený fakt.',userImpact:'Praktický dopad.'}],connectedTopics:[],blindspots:[],faq:[],plotAndProcess:['Ověřte orientaci na pozemku.'],auditConclusion:'Dům odpovídá vybraným prioritám.',cta:'Domluvte si další krok.'};
...
test('loads and embeds WebP/JPEG/PNG House media, including the 4:3 floorplan', async () => {
  ...
  const assets = new Map<string, Uint8Array>([
    ...
    ['/house/interior.jpg', new Uint8Array(await source.clone().jpeg().toBuffer())],
  ]);
  ...
  assert.equal(pdf.getPageCount(), 10);
  assert.deepEqual(requested, ['/house/exterior.webp', '/house/floorplan.png', '/house/interior.jpg']);
