...
/** Room Index content — parent supplies asymmetric L/R inset (TOUR-31). */
...
export const SPATIAL_TERMINAL_MEDIA_TERMINAL_CONTENT_CLASS =
  'grid w-[580px] min-w-[580px] max-w-[580px] shrink-0 content-start items-start gap-0 mobile:w-full mobile:min-w-0 mobile:max-w-none';

/**
 * Floor plan column (TOUR-27 / TOUR-31):
 * 40 px from menu, 20 px to section edge; height from plan + min 50 px to toggles.
 */
export const SPATIAL_TERMINAL_FLOOR_PLAN_SECTION_CLASS =
  'relative z-0 flex h-full w-full min-w-0 flex-col items-stretch overflow-x-hidden pl-[40px] pr-[20px] pb-section';
