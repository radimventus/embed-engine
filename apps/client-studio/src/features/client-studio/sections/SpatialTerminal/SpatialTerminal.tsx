...
export function SpatialTerminal() {
  return (
    <div
      ...
      tabIndex={-1}
      className={`scroll-mt-header grid w-full min-w-0 items-stretch gap-0 overflow-x-hidden ${SECTION_SURFACE_CLASS} grid-cols-1 mobile:px-1 divide-y divide-embed-border-default tabletMin:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] tabletMin:divide-y-0 tabletMax:!grid-cols-[minmax(0,1.55fr)_minmax(12rem,0.55fr)_minmax(0,1fr)] tabletMax:!divide-y-0 desktop:grid-cols-[var(--spatial-terminal-cols)] desktop:divide-y-0 [&>[aria-label='Seznam místností']]:border-embed-border-default desktop:[&>[aria-label='Seznam místností']]:border-r mobile:[&>[aria-label='Seznam místností']]:border-r-0`}
      style={{
