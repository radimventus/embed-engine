...
type SpatialTerminalProps = {
  readonly onBack: () => void;
};

export function TourBackNavigation({ onBack }: SpatialTerminalProps) {
  return (
    <div data-tour-navigation="" className={JOURNEY_CTA_FOOTER_ROW_CLASS}>
      <button
        type="button"
        data-tour-back=""
        onClick={onBack}
        className={`${JOURNEY_CTA_SECONDARY_CLASS} justify-start`}
      >
        ← Zpět
      </button>
    </div>
 ...
 */
export function SpatialTerminal({ onBack }: SpatialTerminalProps) {
  return (
    <>
      <div
      ...
      <TourBackNavigation onBack={onBack} />
    </>
  );
}
