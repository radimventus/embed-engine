...
describe("PT-CS-07 / CAP-PLAT-02c ClientStudioSidebar", () => {
  ...
  it("wires hamburger to house list panel via CPL", () => {
    ...
    assert.match(source, /listWorkspaceHouses/);
    assert.match(source, /switchActiveHouse|updateWorkspaceScope/);
  ...
  it("exposes Scene Navigator dots under hamburger", () => {
    ...
    assert.match(source, /journey-scene-priority/);
    assert.match(source, /journey-scene-racio/);
    assert.match(source, /journey-scene-decision/);
    assert.match(source, /sectionId: PILOT_SECTION_IDS\.hero/);
    assert.match(source, /sectionId: PILOT_SECTION_IDS\.walkthrough/);
    assert.match(source, /activeOrientationSectionId/);
    assert.match(source, /useActiveSection/);
    assert.match(source, /activeSectionId === item\.sectionId/);
    assert.match(
      source,
