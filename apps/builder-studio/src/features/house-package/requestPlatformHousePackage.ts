...
export async function requestPlatformHousePackageInitialize(
  houseId: string,
): Promise<
  | {
      readonly ok: true;
...
> {
  const response = await fetch(endpoint(houseId, "initialize"), {
    method: "POST",
    credentials: "include",
    headers: {
  ...
  if (body.ok !== true || body.houseId !== houseId) {
    return {
      ok: false,
      error:
        "Platform API returned an invalid House Package initialize response.",
    };
  }
...
export async function requestPlatformHousePackagePersist(
  ...
  return typeof body.houseId === "string"
    ...
    : {
        ok: false,
        error: "Platform API returned an invalid House Package response.",
      };
}
...
export async function requestPlatformHousePackageState(
  ...
  if (
    ...
    throw new Error(
      "Platform API returned invalid persisted House Package state.",
    );
  }
